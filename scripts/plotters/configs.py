...
LOG_CONFIG = {
    "flame_fwdllm_aggregator": [
        ...
        {
            'name': 'collect_and_aggregate_grads_latency',
            'regex': re.compile(
                r"^(?P<timestamp>\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2},\d{3}).*?"
                r"\[decorator\]\sRuntime of collect_and_aggregate_grads:\s(?P<runtime>[\d\.]+)s\s"
                r"\(Round=(?P<round_id>\d+),\sDataId=(?P<data_id>\d+),\sIter=(?P<iter_id>\d+),\sTrainerId=(?P<trainer_id>\w+|None)\)"
            ),
            'type': 'EXTRACT',
            'group_to_columns': {
                'timestamp': ('timestamp', lambda ts_str: datetime.strptime(ts_str, '%Y-%m-%d %H:%M:%S,%f')),
                'runtime': ('collect_and_aggregate_grads_latency', float),
                'round_id': ('round_id', int),
                'data_id': ('data_id', int),
                'iter_id': ('iteration_id', int),
                'trainer_id': ('trainer_id', None)
...
EXPORT_CONFIG = {
    ...
    'flame_fwdllm_aggregator': {
        ...
        'collect_and_aggregate_grads_latency': {
            'default_output_filename': lambda: f'{CONSTANTS["file_prefix"]}-collect_and_aggregate_grads_latency.csv',
            'log_names': ['collect_and_aggregate_grads_latency'],
            'columns': ['timestamp', 'collect_and_aggregate_grads_latency', 'round_id', 'data_id', 'iteration_id']
        },
