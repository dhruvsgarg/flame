...
def merge_keys(src_file_path, dst_file_path):
    try:
        ...
        if "hyperparameters" not in src_data:
            print(f"Error: 'hyperparameters' key missing in source {src_file_path}", file=sys.stderr)
            return
        
        if "hyperparameters" not in dst_data:
            print(f"Warning: 'hyperparameters' key missing in dest {dst_file_path}. Creating it.", file=sys.stderr)
        ...
        for key in KEYS_TO_COPY:
            if key in src_params:
                ...
                print(f"Warning: Key '{key}' not found in source {src_file_path}", file=sys.stderr)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: python3 {sys.argv[0]} <source_json_file> <destination_json_file>", file=sys.stderr)
        sys.exit(1)
