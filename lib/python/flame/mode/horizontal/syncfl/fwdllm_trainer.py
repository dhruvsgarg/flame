...
class Trainer(Role, metaclass=ABCMeta):
    ...
    def _fetch_weights(self, tag: str) -> None:
        ...
        if MessageType.VAR in msg:
            logger.info(
                f"Calc more variance received for trainer id: {self.trainer_id} and round {self._round} and model version {self._model_version}. Not updating weights"
            )
        ...
        logger.info(
            ...
            f"round: {self._round}, data id: {self.data_id}, model version: {self._model_version} and work_done: {self._work_done} ###"
        )
    ...
    def _send_grads(self, tag: str) -> None:
        ...
        if self.task_to_perform == "train":
            ...
            logger.info(
                ...
                f"{self._updates_returned_upto_round}, model version { self._updates_returned_upto_model}"
            )
