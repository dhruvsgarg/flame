...
def log_error_distribution(probs, labels):
    ...
    logging.debug(
        "Summary: %d errors have a margin > 0.5 (Confidently Wrong).", high_margin_count
    )
    ...
    def read_trainer_unavailability(self, trace=None) -> None:
        ...
        for file_path in json_files:
            with open(file_path) as f:
                ...
                logger.info(
                    f"Processing file {file_path} for trainer {curr_trainer_id}"
                )
    ...
    def _log_and_reset_model_version_stats(self):
        ...
        def compute_percentiles(values, reverse=False):
            ...
            p1, p5, p20, p30, p50, p75, p90, p99 = (
                (99, 95, 80, 70, 50, 25, 10, 1)
                if reverse
                else (1, 5, 20, 30, 50, 75, 90, 99)
            )
            return (
