...
class Channel(object):
    ...
    def recv(self, end_id) -> tuple[Any, datetime]:
        ...
        fashion."""
        logger.info(f"will receive data from {end_id}")
    ...
    async def _streamer_for_recv_fifo(self, end_ids: list[str], timeout=None):
        ...
        for end_id in end_ids:
            ...
            if end_id in self._active_recv_fifo_tasks:
                ...
                queue_depth = (
                    self._ends[end_id].qsize()
                    if hasattr(self._ends[end_id], "qsize")
                    else "unknown"
                )
                logger.warning(
                    f"[RECV_FIFO] Skipping end_id {end_id} - already has active task, queue_depth={queue_depth}"
                )
    ...
    def drain_messages(self):
        ...
        for end_id in list(self._ends.keys()):
            ...
            while True:
                ...
                _ = self.recv(end_id)
                msg_delete_count += 1
            logger.info(f"Drained {msg_delete_count} messages from end_id {end_id}")
    ...
    def await_join(self, timeout=None) -> bool:
        ...
        async def _inner() -> bool:
            ...
            try:
                ...
                return True
            logger.info("at least one peer joined")
            return False
    ...
    async def add(self, end_id):
        ...
        if end_last_unavail_ts is not None:
            logger.info(f"END_LAST_UNAVAIL_TS was not None for end_id {end_id}")
    ...
    async def remove(self, end_id):
        ...
        logger.warning(
            f"[CHANNEL REMOVE] Removing end {end_id} from channel {self._name}"
        )
        if not self.has(end_id):
            logger.info(
        ...
        if end_last_avail_ts is not None:
            logger.info(f"END_LAST_AVAIL_TS was not None for end_id {end_id}")
