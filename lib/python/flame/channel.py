...
class Channel(object):
    ...
    def set_end_property(self, end_id: str, key: str, value: Scalar) -> None:
        ...
        if self.has(end_id):
            ...
            logger.info(f"SET property {key} with val {value} for end_id {end_id}")
        else:
            logger.info(
    ...
    def get_end_property(self, end_id, key) -> Scalar:
        ...
        if self.has(end_id):
            ...
            logger.info(f"GOT property {key} with val {val} for end_id {end_id}")
            return val
        else:
            logger.info(
    ...
    def ends(
        ...
        async def inner():
            ...
            for end_id, kv in selected.items():
                ...
                logger.info(f"appended end_id {end_id} to id_list, kv is {kv}")
                if not kv:
                    continue
