...
class REFLFedAvg(AbstractOptimizer):
    ...
    def compute_importance_weights(
        self, trainers: List[TrainResult], total: int
    ) -> Dict[str, float]:
        """
        ...
        max_stat_utility = max(
            [tres.stat_utility for tres in trainers if tres.stat_utility], default=1.0
        )
        ...
        for tres in trainers:
            ...
            if self.stale_factor > 1:
                ...
                weight = (1 - self.stale_beta) / (staleness + 1) + self.stale_beta * (
                    1 - math.exp(-client_ratio / max_stat_utility) / self.scale_coff
                )
