...
def import_libs():
    ...
    if args.task == "nlp" or args.task == "text_clf":
        ...
        from fedscale.dataloaders.transforms_stft import (
        ...
        from fedscale.dataloaders.transforms_wav import (
            ChangeAmplitude,
...
parser.add_argument(
    "--data_map_file", type=int, help="path to client-to-data mapping file"
)
...
testing_sets.partition_data_helper(
    num_clients=int(args.num_participants / train_test_ratio) + 1
)
...
with open(train_csv_path, "r") as input_file:
    ...
    for i in range(len(training_sets.partitions)):
        ...
        output_path = os.path.join(
            "/tmp/flame_dataset/", args.data_set, "train", output_filename
        )
        read_lines_and_write_to_csv(
            csv_reader_dict, output_path, training_sets.partitions[i]
        )

        if i % 100 == 0:
            completed += 100
            print(
                f"{completed} partitions completed, {len(training_sets.partitions) - completed} remains..."
            )
...
with open(test_csv_path, "r") as input_file:
    ...
    for i in range(len(testing_sets.partitions)):
        ...
        output_path = os.path.join(
            "/tmp/flame_dataset/", args.data_set, "test", output_filename
        )
        read_lines_and_write_to_csv(
            csv_reader_dict, output_path, testing_sets.partitions[i]
        )

        if i % 10 == 0:
            completed += 10
            print(
                f"{completed} partitions completed, {len(testing_sets.partitions) - completed} remains..."
            )
