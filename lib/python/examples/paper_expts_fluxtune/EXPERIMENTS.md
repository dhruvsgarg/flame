...
|---|---|---|---|---|---|
| `sec:eval:tta` | **E1** time-to-accuracy | fwdllm, fedbuff_round, felix_round, fluxtune | manifest `figs_main_v2.yaml` (`PLOT_TRACKER.md` Manifest 1) | ✅ RENDERED | All 4 anchor baselines landed + rendered → `expt_scripts/paper_figs_main_v2/`, wired into `evaluation.tex` via `figs/code/eval/e2e/e1_tta.tex`. Manifest's 5th row `fwdllm_it_oracular` is intentionally not run (confirmed by operator 2026-07-25 — inert under `syn_0`'s 100% availability, identical to `fwdllm_it_unaware`); not one of E1's 4 anchors either way. |
...
| `sec:eval:attribution` | **A0** attribution (reuses E1–E4 metrics) | fluxtune, fwdllm_it_unaware, felix_it, fedbuff_it_unaware (the `_oracular` twins intentionally excluded — see note) | **separate** manifest `figs_attribution.yaml` (`PLOT_TRACKER.md` Manifest 2) — corrects this row's prior "reuses `main_v2`, 2-baseline `fwdllm_it_oracular`" framing, which no longer matches either the manifest or the tex | ✅ RENDERED (4/4) | Refreshed 2026-07-25: `fluxtune`, `fwdllm_it_unaware`, `felix_it`, `fedbuff_it_unaware` (the last promoted off its 0230-batch relaunch `run_20260724_144822`, 79.5% peak, superseding the earlier dead 16-eval attempt) — **all 4 target baselines now render, manifest complete.** Both `_oracular` twins are intentionally not run (confirmed by operator 2026-07-25 — inert under `syn_0`'s 100% availability, identical to their `_unaware` twin already landed), not missing rows. **Open mismatch (unchanged, still needs an operator claims call):** `evaluation.tex`'s prose cites `\fwdllmito{}` (oracular) numbers ("80.9%, 7.4h") but that baseline was never going to run under this trace — the landed, correct comparison point is `fwdllm_it_unaware` (85.26% @ 1.56h). Flagged inline as a `\tbd` in `figs/code/eval/attr/a_tta.tex`; not resolved here since it's a claims/numbers decision, not a wiring fix. |
...
**Sim N=100 α=1 batch, launched 2026-07-24 ~02:30 onward** (verified from `aggregator_config.json`'s
`partition_method: niid_label_clients=100_alpha=1`, agg_goal=10; all `syn_0`). This is the long-running
sim-mode run-set feeding `figs_main_v2.yaml`/`figs_attribution.yaml` (`PLOT_TRACKER.md` Manifests 1-2) —
**not** the short (~1-2h) post-17:30 real-sim parity batch below, which reuses some of the same baseline
names on different run dirs; the two are disjoint by launch time, kept that way deliberately.

| Run dir | Baseline | Node | Condition | Verdict | Feeds | Notes |
|---|---|---|---|---|---|---|
| `run_20260724_023627_fwdllm_n100_smoke_syn_0_sim` | fwdllm | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 84.97% @ 2.13h | `figs_main_v2.yaml` E1-E5 | — |
| `run_20260724_093607_fedbuff_round_n100_smoke_syn_0_sim` | fedbuff_round | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 85.54% @ 4.18h | `figs_main_v2.yaml` E1-E5 | supersedes dead `run_20260724_000757_…` (0 evals) |
| `run_20260724_042943_felix_round_n100_smoke_syn_0_sim` | felix_round | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 84.63% @ 3.95h | `figs_main_v2.yaml` E1-E5 | supersedes dead `run_20260724_015555_…` (11 evals) |
| `run_20260724_023501_fluxtune_n100_smoke_syn_0_sim` | fluxtune | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 84.72% @ 0.80h | `figs_main_v2.yaml`+`figs_attribution.yaml` E1-E5 | also feeds A0 |
| `run_20260724_053835_fwdllm_it_unaware_n100_smoke_syn_0_sim` | fwdllm_it_unaware | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 85.26% @ 1.56h | `figs_attribution.yaml` A0 | — |
| `run_20260724_092656_felix_it_n100_smoke_syn_0_sim` | felix_it | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 84.49% @ 2.35h | `figs_attribution.yaml` A0 | — |
| `run_20260724_144822_fedbuff_it_unaware_n100_smoke_syn_0_sim` | fedbuff_it_unaware | — | N=100, syn_0 sim, α=1, agg_goal=10 | SMOKE, peak 79.51%, 5.05h span | `figs_attribution.yaml` A0 | landed 2026-07-25 sync; supersedes dead `run_20260724_001229_…` (16 evals, 39.5%) |
| _skipped, by design_ | fwdllm_it_oracular | — | N=100, syn_0 sim, α=1 | intentionally not run | `figs_main_v2.yaml`+`figs_attribution.yaml` | confirmed by operator 2026-07-25 — oracular tracking is inert under `syn_0`'s 100% availability, identical to the landed `fwdllm_it_unaware`; not a gap |
| _skipped, by design_ | fedbuff_it_oracular | — | N=100, syn_0 sim, α=1 | intentionally not run | `figs_attribution.yaml` A0 | same reason. A short post-17:30 sim run exists but belongs to the separate parity batch below, not reused here |
