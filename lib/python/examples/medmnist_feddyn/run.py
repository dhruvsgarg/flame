...
# aggregator config
update_params(
    "./aggregator/template.json",
    "./output/config/aggregator.json",
    job=job,
    taskid=new_randhex(used=used_task_ids, length=40),
)

# trainer config
for index, site in enumerate(sites):
    update_params(
        "./trainer/template.json",
        f"./output/config/trainer{index+1}.json",
        job=job,
        taskid=new_randhex(used=used_task_ids, length=40),
        dataset=site,
    )
...
for i in range(1, len(sites) + 1):
    ...
    os.system(
        f"python ../../../trainer/main.py ../../config/trainer{i}.json > log{i}.txt 2>&1 &"
    )
    os.chdir("..")
