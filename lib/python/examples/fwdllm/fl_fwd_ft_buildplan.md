...
| # | node | task | done when |
|---|---|---|---|
| **F′** | 1 GPU, ~5–7 h | **RUN FIRST. yelp-p controller, `anchor` + `log_only`, 50,000 vclock.** One arm answers three things: does `anchor` keep `ρ*` alive to target accuracy · is the `Φ`≈2.7 rail real under law C (both stops log, nothing halts) · and it produces the **accuracy-vs-`Λ` curve past the stop that sizes row E**. Longer than `125010`'s 3.3 h because `ρ*` stays ~1.7× higher and nothing halts it. `P4_BMAX_POLICY=anchor P4_PHI_STOP=log_only` | an accuracy curve through and past `Φ`=2.7, and whether it turns |
| **E** | any CPU, **while F′ runs** | **Build the saturation stop — it is now the primary termination rule.** Prechelt generalization-loss / patience on *smoothed* held-out accuracy (11-eval trailing window, as `Φ` already is — a raw `dAcc/dΛ` slope false-triggers on a dip and misses a masked plateau). Track `Acc_best` as a running max; `GL_t = (Acc_best − Acc_t)/Acc_best`; stop when `GL_t` holds above a threshold for a patience window. Ships as `stop = saturation OR Φ-cross`. Resample onto `Λ`, never `comm_round`. **Size it on F′'s curve** — it is the only arm that will have run past its own plateau | window, threshold and patience sized **by replay**, reproducing a stop on F′ at its plateau. `replay_phi_stop.py` is the model to copy |
| **C** | any GPU node, **after** F′ + E | **agnews controller**, 48,000 vclock, on the new stack (`anchor`, saturation-primary). `condition_fp` will no longer read `c2ef1528` — that is expected and correct; control `021843` does not run the probe, so it stays the valid partner. `152215` reached 87.9% of `B_max` in 1.85 h | ends on `[BudgetStop] reason=saturation`, at or above 0.850 |
...
| **D** | any GPU node, after **D0** + F′ + E | **yahoo controller**, 60,000 vclock, same new stack. The dataset furthest from its reference (0.657 of 0.734) and the one climbing fastest when killed, so it has the most to gain from `anchor` | ends on saturation, at or above 0.734 |
| **P** | 1 GPU, ~30 min | **Re-range the `B_max` probe's Φ grid.** `(1.5 … 4)` was sized off B-1's *offline* knees; live, 5 of 8 sampled fires put the knee **below the whole grid** and 2.5–4.0 were used on none. One short agnews arm at `--b-max-probe-every 150` with a dense low grid — `1.05,1.1,1.2,1.3,1.5,2.0` — reaching ~200 commits fires one probe and reads where the knee actually is. Nothing on disk answers it offline: no arm checkpoints a model. `b_max_probe_phis` is read from `args` and set nowhere, so it needs plumbing — and **`condition_fp` does not cover it**, the same blind spot the sim-charge profile had (§4.5) | a knee bracketed by real grid points, and a grid sized by that measurement |
