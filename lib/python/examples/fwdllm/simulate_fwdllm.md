...
- **#1c/K-D27 R1 two-ledger bridge (2026-07-05)** — async_oort gated selection eligibility ONLY on `all_selected`
  ...
  SHRINKS (committed → re-pickable); the old `|= outstanding` would starve. Sim-only (∅ in real → async_cifar10
  byte-identical). `TestPendingCommitExcludedFromSelection` + `TestPendingCommitBridge`; full `tests/` 886-green.
...
- **K-D27** — #1c R1 DEEPER root-cause from the post-#14 `_2327` telemetry (part (a) recovered ~0; R1 still
  ...
  `SIM_R1_DISPATCH → 0`, R1 overlap → ~0.2%. **Awaiting the fluxtune sim re-run to bank the recovery.**
