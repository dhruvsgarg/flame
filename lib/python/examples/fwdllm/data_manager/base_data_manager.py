...
class BaseDataManager(ABC):
    ...
    def _load_federated_data_server(self, test_only=True, test_cut_off=None):
        ...
        if state:
            ...
            if not test_only:
                logging.info(f"NRL: len of train_index_list: {len(train_index_list)}; train_index_list: {train_index_list}")
            logging.info(f"NRL: len of test_index_list: {len(test_index_list)};test_index_list: {test_index_list}")
