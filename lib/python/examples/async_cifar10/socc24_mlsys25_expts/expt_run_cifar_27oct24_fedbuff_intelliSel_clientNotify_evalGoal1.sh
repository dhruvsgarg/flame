...
for baseline_name in "${baseline_names[@]}"; do
  ...
  for alpha in "${alphas[@]}"; do
    ...
    agg_log_file="/Users/seshutummala/CodeRepo/OMSCS//flame/lib/python/examples/async_cifar10/aggregator/agg_${node_name}_$(date +%d_%m_%H_%M)_alpha${alpha}_cifar_70acc_${aggType}_${selType}_${awareType}_${baseline_name}_50.log"
    echo "Created aggregator log file: ${agg_log_file}"
    python pytorch/main.py expt_cifar_27oct24_fedbuff_iSel_clientNotify_evalGoal_1_c30_k10.json --log_to_wandb --wandb_run_name agg_${node_name}_$(date +%d_%m_%H_%M)_alpha${alpha}_cifar_70acc_${aggType}_${selType}_${awareType}_${baseline_name}_battery50_evalGoal1_c30_k10 > "$agg_log_file" 2>&1 &
    ...
    trainer_log_file="/Users/seshutummala/CodeRepo/OMSCS//flame/lib/python/examples/async_cifar10/trainer/config_dir${alpha}_num300_traceFail${trainer_dir_suffix}/log_trainer_${node_name}_$(date +%d_%m_%H_%M)_${alpha}_${aggType}_${selType}_${awareType}_${baseline_name}_50.log"
