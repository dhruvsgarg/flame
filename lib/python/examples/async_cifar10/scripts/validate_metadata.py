...
class MetadataValidator:
    ...
    def validate_all(self) -> bool:
        ...
        if all_passed:
            ...
            print("  3. Git commit: 'Phase 1 complete: Extracted and validated metadata'")
        else:
    ...
    def _validate_dataset_splits(self) -> bool:
        ...
        for split_name, split_data in sorted(self.dataset_splits.items()):
            ...
            if errors_in_split == 0:
                ...
                print(f"      ✓ {num_trainers} trainers (sampled {len(sample_trainers)})")
            else:
                passed = False
    ...
    def _validate_synthetic_traces(self) -> bool:
        ...
        print(f"  Validating {len(self.synthetic_traces['traces'])} synthetic traces...")
        ...
        for trace_name in ['syn_0', 'syn_20', 'syn_50']:
            ...
            json_pattern_as_lists = [list(item) if isinstance(item, tuple) else item for item in json_pattern]
    ...
    def _validate_mobiperf_traces(self) -> bool:
        ...
        print(f"  Validating mobiperf traces (sampling {len(sample_trainer_nums)} devices)...")
        ...
        for trainer_num in sample_trainer_nums:
            ...
            try:
                ...
                for variant in ['2st', '3st_50', '3st_75']:
                    ...
                    # Convert JSON tuples to lists for comparison
                    json_trace_as_lists = [list(item) if isinstance(item, tuple) else item for item in json_trace]
                    
                    if yaml_trace != json_trace_as_lists:
                        self.errors.append(
                            f"{device_id}: mobiperf_{variant} mismatch"
                        )
                        errors_found += 1
            except Exception as e:
        ...
        if errors_found == 0:
            ...
            print(f"    ✓ All {num_devices} device traces valid (sampled {len(sample_trainer_nums)})")
            print(f"      Each device has 3 variants: 2-state, 3-state-50%, 3-state-75%")
        else:
            passed = False
    ...
    def _validate_completeness(self) -> bool:
        ...
        for split_name, split_data in self.dataset_splits.items():
            ...
            if actual != expected:
                self.errors.append(
                    f"{split_name}: Has {actual}/{expected} trainers"
                )
                incomplete_splits += 1
        
        if incomplete_splits == 0:
            print(f"    ✓ All {len(self.dataset_splits)} splits complete (300 trainers each)")
        else:
            passed = False
