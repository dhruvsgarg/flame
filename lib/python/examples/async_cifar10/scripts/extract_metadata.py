...
class MetadataExtractor:
    ...
    def _scan_all_configs(self):
        ...
        print(f"  ✓ Loaded {total_files} config files from {len(self.all_configs)} directories:")
    ...
    def _extract_dataset_splits(self):
        ...
        for split_name, split_data in sorted(self.dataset_splits.items()):
            ...
            total_samples = sum(len(indices) for indices in split_data['trainer_data_splits'].values())
            print(f"    • {split_name}: {num_trainers} trainers, {total_samples} total samples")
    ...
    def _extract_traces(self):
        ...
        print(f"    (Each device has 3 trace variants: 2-state, 3-state-50%, 3-state-75%)")
