...
usage() {
  echo "usage: $0 --node node1|node2 [--baselines 'felix refl'] [--runtime-s 3600]"
  echo "       $0 smoke"
  exit 2
}
...
# $4 = output path, [$5 = smoke: 1|0]
make_debug_yaml() {
  python - "$SCR" "$1" "$2" "$3" "$4" "${5:-0}" <<'PY'
import yaml, sys, copy
scr, node, baselines_str, runtime_s, outpath, smoke = (
...
# ---- normal run mode ----
echo "=== DEBUG RUN: node=$NODE baselines='$BASELINES' runtime_s=$RUNTIME_S ==="
cfg="$LOGDIR/debug_${NODE}.yaml"
make_debug_yaml "$NODE" "$BASELINES" "$RUNTIME_S" "$cfg" 0
