...
class ExperimentRunner:
    ...
    def _build_trainer_spawn_command(self, exp_config: ExperimentConfig) -> list:
        ...
        return [
            ...
            '--alpha', str(exp_config.trainer.dataset.dirichlet_alpha),
            '--availability', exp_config.trainer.availability.mode,
            '--num-trainers', str(exp_config.trainer.num_trainers),
            '--start-id', str(exp_config.trainer.start_id),
            '--num-gpus', str(exp_config.execution.num_gpus),
        ]
