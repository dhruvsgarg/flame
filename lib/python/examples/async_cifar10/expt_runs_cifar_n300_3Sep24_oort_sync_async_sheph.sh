...
for baseline_name in "${baseline_names[@]}"; do
  ...
  for alpha in "${alphas[@]}"; do
    ...
    python pytorch/main.py fedbuff_config_final_expt_3sep24_${baseline_name}_felix_v2.json --log_to_wandb --wandb_run_name agg_${node_name}_$(date +%d_%m_%H_%M)_alpha${alpha}_cifar_70acc_oort_${baseline_name} > "$agg_log_file" 2>&1 &
