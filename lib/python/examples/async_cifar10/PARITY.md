...
>
> **Next run = SEEDED 2 h real+sim pairs** (user shortened the loop). Read `Sdet` first: it tells you, per baseline, whether seeding closed the gap or where it didn't. felix also still has the paper-defaults async-overlap regression (below) — a seeded felix isolates how much of its advance-collapse is selection-mix (should recover) vs the clock model (won't).
