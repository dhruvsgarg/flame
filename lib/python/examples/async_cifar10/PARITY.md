...
>
> **Short-run guard (so the checker can't mislead):** C1/C2 use `SHORT_RUN_CONFIDENCE_S = 7200` (2 h). On a sub-2 h budget a convergence **PASS** is downgraded to **`[??] LOWC`** (`low_confidence`, one-sided — the curves haven't diverged yet); a genuine **FAIL still surfaces** unchanged (a short-run divergence is real). `≥2 h` PASS stays a confident `[OK]`. `_mark_low_confidence_if_short` in `checks.py`, guard `test_convergence_low_confidence_on_short_runs`.
