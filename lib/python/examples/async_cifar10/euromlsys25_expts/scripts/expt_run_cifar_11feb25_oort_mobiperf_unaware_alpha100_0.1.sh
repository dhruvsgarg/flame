...
check_accuracy() {
  ...
  if [ "$elapsed_time" -ge 14400 ]; then
    echo "4 hours have elapsed. Terminating."
...
for baseline_name in "${baseline_names[@]}"; do
  ...
  for alpha in "${alphas[@]}"; do
    ...
    python pytorch/main_oort_agg.py oort_config_large_expt_unaware_11feb25_mobiperf.json --log_to_wandb --wandb_run_name agg_${node_name}_$(date +%d_%m_%H_%M)_alpha${alpha}_cifar_70acc_${aggType}_${selType}_${awareType}_c13_1.3k > "$agg_log_file" 2>&1 &
    agg_pid=$!
    ...
    while true; do
      if check_accuracy "$agg_log_file" "$threshold" "$start_time"; then
